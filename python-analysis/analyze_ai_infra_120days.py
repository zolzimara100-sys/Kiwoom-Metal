...
def analyze_ai_infra_120days():
    ...
    try:
        ...
        # 2. 20251210 기준 120거래일 데이터 조회
        print("\n" + "=" * 100)
        print("2. 120거래일 날짜별 외국인 투자자 금액 합계")
        print("   계산식: sum(cur_prc * frgnr_invsr)")
        print("   기간: 20251210 기준 최근 120거래일")
        print("=" * 100)
        
        # 20251210 기준으로 120거래일 데이터 조회
        query_120days = f"""
            SELECT dt, 
        ...
        print("-" * 60)
        print(f"{'#':<5} {'날짜':<15} {'합계 (cur_prc * frgnr_invsr)':<30}")
        print("-" * 60)
        ...
        for idx, row in enumerate(daily_data_sorted, 1):
            ...
            # 마지막 10일과 처음 10일만 표시, 중간은 생략
            if idx <= 10 or idx > len(daily_data_sorted) - 10:
                print(f"{idx:<5} {dt_str:<15} {formatted_amount:>30}")
            elif idx == 11:
                print(f"{'...':<5} {'...':<15} {'... (중간 생략) ...':<30}")
        
        # 3. 120일 이동평균 계산
        print("\n" + "=" * 100)
        print("3. 20251210 기준 120일 이동평균 (MA120) 계산")
        print("=" * 100)
        
        if count > 0:
            ma_120 = total_for_ma / count
            print(f"\n거래일 수: {count}일")
            print(f"합계: {total_for_ma:,.0f}원")
            print("-" * 60)
            print(f"\n★ 120일 이동평균 (MA120) = {total_for_ma:,.0f} / {count}")
            print(f"                        = {ma_120:,.0f}원")
