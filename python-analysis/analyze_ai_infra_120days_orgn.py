"""
AI Infra 섹터 종목 120일 분석 - 기관 투자자 (orgn)
"""
...
def analyze():
    ...
    print("=" * 100)
    print("AI Infra 섹터 종목 120일 분석 - 기관 투자자 (orgn)")
    print("계산식: sum(cur_prc * orgn)")
    print("=" * 100)
    ...
    cursor.execute(f"""
        SELECT dt, SUM(cur_prc * orgn) as total_amount
        FROM tb_stock_investor_chart
        WHERE stk_cd IN ({stock_codes_str}) AND dt <= '20251210'
        GROUP BY dt ORDER BY dt DESC LIMIT 120
    """)
    ...
    for idx, (dt, amount) in enumerate(daily_data, 1):
        ...
        if idx <= 10 or idx > len(daily_data) - 10:
    ...
    print(f"\n★ 120일 이동평균 (MA120) - 기관 = {total:,.0f} / {count}")
    print(f"                                = {ma:,.0f}원")
